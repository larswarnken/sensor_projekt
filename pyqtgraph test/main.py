...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def check_if_recording(self):
        ...
        while True:
            if not os.path.isfile(f'c++/{output_path["outputPath"]}'):
                continue
            else:
                print("recording...")
        ...
        while True:
            time.sleep(0.5)
            current_file_size = os.path.getsize(f'c++/{output_path["outputPath"]}')
    ...
    def load_data(self):
        ...
        with open(f'c++/{output_path["outputPath"]}', 'r') as file:
            details = file.read()
            x = details.split('\n')
        try:
        ...
        while self.currently_recording:
            ...
            if self.loaded_data is not None:
                if len(self.loaded_data) <= 10000:
                    ...
                    self.x = list(range(len(self.loaded_data) - 10000, len(self.loaded_data)))
                    self.y = self.loaded_data[len(self.loaded_data) - 10000:len(self.loaded_data)]
